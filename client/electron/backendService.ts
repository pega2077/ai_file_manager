...
export const startPythonServerWithConfig = (config: AppConfig): void => {
  ...
  // 使用配置文件中的路径
  const servicePath = path.isAbsolute(config.localServicePath)
    ? config.localServicePath
    : path.join(projectRoot, config.localServicePath);

  const exePath = path.isAbsolute(config.localServicePythonExe)
    ? config.localServicePythonExe
    : path.join(projectRoot, config.localServicePythonExe);

  logger.info('Starting Python server with config:', {
    servicePath,
    exePath,
    host: config.localServiceHost,
  ...
  const env = {
    ...process.env,
    HOST: config.localServiceHost,
    PORT: config.localServicePort.toString()
  };

  // 使用配置中的 Python 解释器和脚本路径启动
  pythonProcess = spawn(exePath, [servicePath], {
    cwd: projectRoot,
    detached: false,
    stdio: ['ignore', 'pipe', 'pipe'],
    env
